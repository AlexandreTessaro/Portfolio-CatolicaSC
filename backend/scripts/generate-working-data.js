...
async function generateWorkingData() {
  console.log('🔄 Iniciando geração de dados sintéticos funcionais...');
  ...
  try {
    ...
    console.log('\n🎉 Geração de dados sintéticos funcionais concluída!');
    console.log('\n📊 Estatísticas finais:');
    ...
    client.release();
    // Não fechar o pool aqui para evitar problemas
    console.log('✅ Cliente liberado, pool mantido ativo');
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generateWorkingData()
    .then(() => {
      console.log('✅ Geração de dados sintéticos funcionais executada com sucesso');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ Falha na geração de dados sintéticos funcionais:', error);
      process.exit(1);
    });
}

export default generateWorkingData;
